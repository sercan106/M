...
def yaklasanodeme(request):
    ...
    for detay in ödendimi_false_öğeler:
        ...
        düzensiz_listesi.append({
            'tamad': f"{detay.sahibi} ({detay.kategori})",
    ...
    toplam_düzenli_tutar = sum(float(detay['tutar']) for detay in düzenliödeme_listesi)
    toplam_düzensiz_tutar = sum(float(detay['tutar']) for detay in düzensiz_listesi)

    
    print(toplam_düzenli_tutar)
    print(toplam_düzensiz_tutar)



    düzensiz_listesi = sorted(düzensiz_listesi, key=lambda x: x['kalangün'])
    birlesik_liste=düzenliödeme_listesi+düzensiz_listesi
    birlesik_liste = sorted(birlesik_liste, key=lambda x: x['kalangün'])

    
    return render(request, 'wejegeh/yaklasanodeme.html', {
        'yaklasanodeme': birlesik_liste,
    })
...
def gelir(request):
    bugun = datetime.now()
    tum_birim_kodlari = [birim[0] for birim in Gelir.BIRIM_CHOICES]
    kaynaklar = ["Sercan", "Mehmet", "Eren"]  # İlgili kaynakları listeleyin

    toplam_gelirler = {}

    for kaynak in kaynaklar:
        toplam_gelirler[kaynak] = {}  # Her bir kaynağın toplam gelirlerini saklamak için iç içe geçmiş bir sözlük

        for birim_kodu in tum_birim_kodlari:
            toplam_gelir = Gelir.objects.filter(
                birim=birim_kodu,
                sahibi__başlık=kaynak,
                tarih__month=bugun.month
            ).aggregate(toplam=Sum('miktar'))

            toplam_gelirler[kaynak][birim_kodu] = toplam_gelir["toplam"] or 0

    return render(request, 'wejegeh/gelir.html', {'toplam_gelirler': toplam_gelirler})
    ...
    for i in düzenliödeme:
        for detay in i.detay.all():
            if detay.ödendimi is False:
                ...
                düzenliödeme_listesi.append({
                    'sahibi': i.sahibi,
                    'kategori': f"{i.sahibi} ({i.kategori})",
                    'toplamtaksit': i.toplamtaksit,
                    'notekle': i.notekle,
                    'taksitsayısı': detay.kaçıncı_taksit,
                    'tutar': detay.tutar,
...
def genel(request):
    ...
    for detay in ödendimi_false_öğeler:
        ...
        düzensiz_listesi.append({
            'sahibi': detay.sahibi,
            'kategori': f"{detay.sahibi} ({detay.kategori})",
            'notekle': detay.notekle,
            'tutar': detay.tutar,
            'ödemeson': detay.ödemeson,
            'ödendimi': detay.ödendimi,
...
def kart_detay(request, sahibi, kategori):
    ...
    if sahibi == "Sercan":
        ...
        veri2 =Kartlar.objects.filter(sahibi__başlık=sahibi, kategori__kategori=kategori)
