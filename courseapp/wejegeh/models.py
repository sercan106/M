...
class Düzenlikategori(models.Model):
    kategori_adı= models.CharField(max_length=100)#zorunlu
    def __str__(self):
        return self.kategori_adı
    
class Ödemedetay(models.Model):
    sahibi = models.ForeignKey(Sahibi,on_delete=models.CASCADE)
    üstkategorisi = models.ForeignKey(Düzenlikategori,on_delete=models.CASCADE)
    kaçıncı_taksit = models.CharField(max_length=100,blank=True)
    ...
    def __str__(self):
      return f"{self.üstkategorisi.kategori_adı} {self.kaçıncı_taksit}. taksit" 

class DüzenliÖdeme(models.Model):
    sahibi = models.ForeignKey(Sahibi,on_delete=models.CASCADE)
    kategori = models.ForeignKey(Düzenlikategori,on_delete=models.CASCADE)
    ...
    def __str__(self):
        return self.kategori.kategori_adı
    
    
    def save(self, *args, **kwargs):
        if not self.slug:
            self.slug = slugify(self.kategori.kategori_adı)
...
class Gelir(models.Model):
    ...
    def __str__(self):
        return f"{self.sahibi} - {self.miktar} {self.get_birim_display()}"        
